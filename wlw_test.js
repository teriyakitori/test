...
function level_senkou_hyouji(){
	if(window.confirm("注意：テスト機能のため、結果や動作のチェックが甘いです。\nLv先行有利を確認するための機能です。\nデータの都合上、レベルアップ時間は最大8秒ほどの誤差がありえます")){
		...
		//addNode("", "勝率(勝利数/敗北数)", 1, "opt");
		addNode("自軍", level_result[5] + "%(" + level_result[0] + "勝/" + level_result[1] + "敗)", 10, "opt");
		addNode("敵軍", level_result[6] + "%(" + level_result[2] + "勝/" + level_result[3] + "敗)", 11, "opt");
